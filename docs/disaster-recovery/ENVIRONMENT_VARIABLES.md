...
| `BACKUP_PGHOST` | Supabase Connect panel / local operator | Private connection metadata | Database yes | Backup process environment | QA direct/session endpoints; owner only the pinned direct hostname. Other owner/pooler hosts rejected until separately verified and pinned. |
...
| `BACKUP_PGUSER` | Supabase Connect panel / local operator | Private connection metadata | Database yes | Backup process environment | Direct postgres user or existing QA session pooler postgres.<QA-ref>; mismatch rejected. |
...
Owner selection requires BACKUP_SOURCE=owner, BACKUP_EXPECTED_REF=zhqmlpljloumldaczcfp, BACKUP_PGHOST=db.zhqmlpljloumldaczcfp.supabase.co, BACKUP_PGUSER=postgres and BACKUP_STORAGE_URL=https://zhqmlpljloumldaczcfp.supabase.co. These are identifiers, not credentials. The owner pooler remains disabled until its exact host is verified; no generic pooler override is provided. Supply password, Storage credential and independent encryption key only through temporary environment injection from protected custody.
