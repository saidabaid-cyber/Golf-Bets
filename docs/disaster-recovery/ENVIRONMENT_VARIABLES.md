...
Every PostgreSQL tool receives `PGOPTIONS=-c default_transaction_read_only=on -c transaction_read_only=on`. Preflight runs `SHOW default_transaction_read_only;` and `SHOW transaction_read_only;` via psql without startup files. Both must return exactly on or the component aborts. This is a separate probe connection; the exporters use the same forced settings, not a claim that their internal connection was queried separately. Both checks repeat before pg_dumpall exports roles. DB/Storage cannot start without BACKUP_ENCRYPTION_KEY. No real owner backup is executed by the allowlist change itself.
