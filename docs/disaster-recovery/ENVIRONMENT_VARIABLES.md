...
| `BACKUP_EXPECTED_REF` | Backup operator | Public identifier | Remote backup yes | Backup process environment | Must match the hard-coded ref for qa or owner, never an arbitrary project override. |
| `BACKUP_PGDATABASE` | DB operator | Private connection metadata | Optional | Backup process environment | Defaults to postgres; remote sources require postgres. Local accepts a plain database name, never a libpq URI/connection string. |
| `BACKUP_PGHOST` | Supabase Connect panel / local operator | Private connection metadata | Database yes | Backup process environment | QA direct endpoint only. Owner permits its exact direct host or only `aws-0-us-east-1.pooler.supabase.com`; every other pooler is rejected. |
...
| `BACKUP_PGPORT` | Supabase Connect panel / local operator | Tooling config | Optional | Backup process environment | Remote sources require 5432; transaction pooler 6543 rejected. Local may use a different valid port. |
| `BACKUP_PGSSLROOTCERT` | DB operator | Certificate file path | Optional | Backup process environment | Defaults to system trust with verify-full for QA/owner; use verified CA bundle if required, not TLS disable. |
| `BACKUP_PGUSER` | Supabase Connect panel / local operator | Private connection metadata | Database yes | Backup process environment | Direct endpoints require `postgres`; the owner Session Pooler requires exactly `postgres.zhqmlpljloumldaczcfp`. Crossed or alternate identities are rejected. |
...
| `BACKUP_SOURCE` | Backup operator | Tooling config | DB/Storage yes | Backup process environment | Only qa, local or explicit owner accepted. local applies to PostgreSQL only; owner is one hard-coded project, not a Production bypass. |
| `BACKUP_STORAGE_KEY` | Supabase API settings / owner vault | SECRET process only | Storage yes | Backup process environment | Modern sb_secret_ server key preferred; authorized legacy service-role credential also supported. Never inferred from app .env. Used only for list/download. |
| `BACKUP_STORAGE_URL` | Supabase API settings | Public identifier | Storage yes | Backup process environment | Exact HTTPS origin for the chosen QA/owner ref; no trailing slash/path/query, cross-origin requests or redirects. |
...
Install PostgreSQL 17-compatible clients (pg_dump, pg_restore, pg_dumpall, psql). BACKUP_* values are inherited by the Node process; scripts do not source any application env file. Passwords are supplied only via process environment, never arguments. The PostgreSQL child does not inherit BACKUP_* secrets or arbitrary PG* options. Host/ref/user/database/port must match the hard-coded source policy. Remote connections require TLS hostname verification.

Owner selection requires `BACKUP_SOURCE=owner`, `BACKUP_EXPECTED_REF=zhqmlpljloumldaczcfp`, `BACKUP_STORAGE_URL=https://zhqmlpljloumldaczcfp.supabase.co` and one exact PostgreSQL tuple: direct `BACKUP_PGHOST=db.zhqmlpljloumldaczcfp.supabase.co` with `BACKUP_PGUSER=postgres`, or Session Pooler `BACKUP_PGHOST=aws-0-us-east-1.pooler.supabase.com` with `BACKUP_PGUSER=postgres.zhqmlpljloumldaczcfp`. Both require port 5432 and database `postgres`. These are identifiers, not credentials. No other pooler/source/ref/user combination or generic override is accepted. Supply password, Storage credential and independent encryption key only through temporary environment injection from protected custody.

Inherited PostgreSQL routing and startup options, including every case variant of `PGOPTIONS`, are removed. The preflight uses one psql connection without startup files to run `BEGIN TRANSACTION READ ONLY; SHOW transaction_read_only; ROLLBACK;`; the SHOW result must be exactly `on`, otherwise the component aborts before export. The preflight repeats before `pg_dumpall --roles-only`. It is a separate probe connection and does not claim that exporter connections inherited a session setting; no `PGOPTIONS` or `default_transaction_read_only` override is injected. DB/Storage cannot start without BACKUP_ENCRYPTION_KEY. No real owner backup is executed by the allowlist change itself.
