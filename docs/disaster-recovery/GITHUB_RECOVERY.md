...
git -C /private/restored/worktree checkout infra/disaster-recovery
git -C /private/restored/worktree rev-parse HEAD
```

Use the branch recorded in manifest, not necessarily the example name. Compare SHA with manifest. The verifier tests the bundle from an empty Git repository so hidden object prerequisites cannot be supplied by the original repo.
