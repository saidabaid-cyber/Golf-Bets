...
The repository now contains the guarded scheduled GitHub Actions workflow `.github/workflows/automated-offsite-backup.yml`. Read-only evidence records one successful scheduled run and workflow artifact; that proves the scheduler produced a bounded backup artifact, not independent custody in Drive or a successful full restore. No Supabase local stack or Docker deployment was found. The remaining acceptance gate is an authorized disposable restore drill plus custody/RPO/RTO verification, not activation of another scheduler.
