...
Historical evidence: repository at initial SHA `dbf30b080c8e76d3e56bab5c311a60e7ac65d897`; read-only QA SQL/management metadata and Vercel project inspection on 2026-09-21. [QA_METADATA_2026-09-21.json](QA_METADATA_2026-09-21.json) contains dated counts and migration identifiers, not user rows or secrets. Current canonical state and later CLI observations are in `../CANONICAL_MIGRATION_LEDGER_2026-09-24.md`; no Production inspection was performed during consolidation.
...
- `supabase/migrations/`: 48 canonical SQL migration files at the 2026-09-24 inventory; do not equate the file count with the remote ledger.
...
Relevant historical refs include main, beta, phase2/full-platform, phase2/course-catalog-feedback and infra/disaster-recovery; the current source of truth is `integration/backyard-current`. Fetch all refs before independent mirroring; do not merge branches during recovery. Current `vercel.json` disables unspecified branches and enables the canonical branch. The repository includes the offsite-backup Actions workflow. No Vercel cron or Next rewrite/redirect configuration was found.
...
Public schema: 45 functions, 84 triggers, 310 policies. Managed schema RLS counts are not an instruction to change those schemas. Extensions: plpgsql, pg_stat_statements, uuid-ossp, pgcrypto, supabase_vault, pg_net, http, pg_trgm. No `pg_cron` extension or Supabase-internal application schedule was present in the 2026-09-21 database snapshot. Separately, the repository now has an external GitHub Actions offsite-backup schedule at `0 9 * * *`, with one observed successful run; that scheduler is covered by the automation runbook and does not imply a completed restore drill.

The 2026-09-21 snapshot had **38 ledger entries vs 30 repository migrations**. Canonical source now has 48 migration files and later live observations found additional divergence; never replay either set blindly. See `../CANONICAL_MIGRATION_LEDGER_2026-09-24.md` and SUPABASE_RECOVERY.
...
Known domains: dev.thebackyard.com.mx (canonical QA target), app.thebackyard.com.mx, beta.thebackyard.com.mx, thebackyard.com.mx and auth.thebackyard.com.mx (email sender domain). The 2026-09-21 snapshot did not establish registrar, authoritative record values, billing owners, or DNS credentials; later consolidation observed that the `dev` record was still absent. Do not infer credentials or repoint any domain from this inventory.
