...
4. Review `vercel.json`: the current rule disables unspecified branches and enables `integration/backyard-current`; recovered backup/hotfix paths remain explicitly disabled. Do not remove release protections just to trigger a deploy.
