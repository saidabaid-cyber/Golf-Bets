...
- `BACKUP_SOURCE=qa` plus `BACKUP_EXPECTED_REF=bymeopxkxapfizeeqeyb`: existing isolated QA source, PostgreSQL direct endpoint only.
...
Owner PostgreSQL permits exactly two host/user tuples on port 5432 with database `postgres`: direct `db.zhqmlpljloumldaczcfp.supabase.co` + `postgres`, or Session Pooler `aws-0-us-east-1.pooler.supabase.com` + `postgres.zhqmlpljloumldaczcfp`. Owner Storage remains pinned to `https://zhqmlpljloumldaczcfp.supabase.co`. No free-form allowlist override or generic pooler pattern exists, and no other source may use a Session Pooler. The direct endpoint remains available where IPv6/direct connectivity works; the exact Session Pooler is the IPv4 alternative.

Before the database/schema export and again before the roles export, `psql -X` uses one connection to run `BEGIN TRANSACTION READ ONLY; SHOW transaction_read_only; ROLLBACK;`; the SHOW result must be exactly `on` or the export aborts. Inherited libpq routing and startup options are removed, and no `PGOPTIONS` override or `default_transaction_read_only` assumption is used. The preflight is a separate connection; the only remaining database processes are `pg_dump`, local `pg_restore` transformation and `pg_dumpall --roles-only`. No restore, source mutation, migration or deployment is performed. This allowlist change does **not** mean a real owner backup has been executed or verified.
