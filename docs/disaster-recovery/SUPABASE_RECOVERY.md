...
Before connecting, the script validates source/ref/host/user/database/port, removes inherited libpq routing/options and requires encryption. Before the database/schema dump and again before the roles dump, one psql connection runs `BEGIN TRANSACTION READ ONLY; SHOW transaction_read_only; ROLLBACK;`; the SHOW result must be exactly `on` or export aborts. The probe does not depend on `default_transaction_read_only` and no `PGOPTIONS` startup override is supplied to exporters. The remaining remote database tools are the read-only `pg_dump` and `pg_dumpall --roles-only`; pg_restore only renders the already encrypted archive into encrypted SQL via stdout (`--file=-`) and never connects to a restore target. No source DDL, writes, Auth/Storage changes or migrations occur. The new owner allowlist is preparation only, not evidence of a live owner backup.
