...
The historical 2026-09-21 snapshot contained 30 local migration files and 38 QA ledger entries. Canonical source now contains 48 files and later live observations found further remote-only/local-only divergence. The following remains historical **order/name reconciliation**, not byte equivalence; use `../CANONICAL_MIGRATION_LEDGER_2026-09-24.md` for current state and never run migration repair merely to make counts agree.
