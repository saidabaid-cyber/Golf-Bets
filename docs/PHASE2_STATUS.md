...
The verified code Preview is complete. Push only this status update to `phase2/full-platform`, wait for its docs-only Preview and repeat a minimal public/API smoke. The exact Round Setup provider payload and the complete post-legal-gate flow remain `PENDING_INTERACTIVE_QA`; physical iPhone remains `PENDING_DEVICE_QA`; private Vercel Runtime Logs remain `BLOCKED_EXTERNAL`; and all eight SQL/RLS/multiuser checks remain `PENDING_CONTROLLED_DB_APPLY` until a distinct Supabase Preview project ref is approved. Do not promote or connect a custom domain.
