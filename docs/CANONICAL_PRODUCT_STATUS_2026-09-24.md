...
| Branch Domain, DNS y deployment canónico | `BLOCKED_EXTERNAL` | `vercel.json` habilita sólo `integration/backyard-current` entre las ramas relevantes y los documentos operativos usan una sola URL objetivo. | Enlazar/verificar `dev.thebackyard.com.mx`, desplegar el SHA final y registrar el deployment ID; no usar URLs aleatorias `*.vercel.app` para owner QA. |
| Browser QA del deployment final | `IMPLEMENTED_NEEDS_INTERACTIVE_QA` | Existen runners de auth, cuenta, cursos, equipo, social y reconciliación, además de `/api/health`. | Ejecutar smoke autenticado, consola/red, assets y binding URL→SHA en la única URL fija. |
...
| La Vista normal Par 72 y layouts temporales Par 70/69 | `IMPLEMENTED_AND_VERIFIED` | P05 confirma identidades/layouts separados y prohíbe copiar Rating/Slope entre ellos. | Verificar selector contra la DB final. |
...
| Secrets server-only y sanitización | `IMPLEMENTED_AND_VERIFIED` | GHIN, Supabase admin, AI y mailers no usan variables públicas; rutas limitan bodies, orígenes, tiempos y errores. | Auditoría de variables/logs del deployment final. |
...
| Vercel sólo despliega la rama canónica relevante | `IMPLEMENTED_AND_VERIFIED` | `vercel.json` habilita `integration/backyard-current` y bloquea ramas de backup/hotfix incluidas. | Verificar configuración remota y branch domain. |
| URL fija y binding a SHA | `BLOCKED_EXTERNAL` | Código, runners y docs esperan `https://dev.thebackyard.com.mx`; no hay evidencia en este corte de DNS/deployment/binding final. | Configurar/verificar fuera del repositorio y registrar deployment ID. |
...
| Console/network QA del Preview final | `IMPLEMENTED_NEEDS_INTERACTIVE_QA` | Runners y checklists existen. | Ejecutar auth, perfil, cursos, ronda, equipo, social, Admin y GHIN-disabled; registrar errores reales. |
