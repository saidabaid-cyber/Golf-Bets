...
import { normalizeRoundStartHole, playOrder, playOrderForHoles } from "../lib/engine";
import { feedbackPersistenceInput, validateFeedback } from "../lib/feedback";
...
test("a 9-hole course rotates only its real holes", () => {
  assert.deepEqual(playOrderForHoles([1, 2, 3, 4, 5, 6, 7, 8, 9], 5), [5, 6, 7, 8, 9, 1, 2, 3, 4]);
  assert.equal(new Set(playOrderForHoles([1, 2, 3, 4, 5, 6, 7, 8, 9], 5)).size, 9);
});
...
test("tee request remains a reported review request with required evidence", () => {
  const input = { category: "TEE", name: "Campo QA", model: "Verde", description: "Tarjeta observada en recepción.", replyEmail: "qa@example.invalid", city: "", state: "", brand: "", rules: "" } as const;
  const checked = validateFeedback(input);
  assert.equal(checked.ok, true);
  if (checked.ok) {
    const persisted = feedbackPersistenceInput(checked.data);
    assert.equal(persisted.category, "COURSE");
    assert.match(persisted.name, /Tee faltante/);
  }
  assert.equal(validateFeedback({ ...input, description: "corto" }).ok, false);
});
