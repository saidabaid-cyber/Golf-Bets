...
test("onboarding, groups and account deletion expose explicit safe choices", () => {
  ...
  assert.match(invitations, /account\.client\.rpc\("group_invitation_action_v1", \{ action: "accept"/);
  const invitationUi = readFileSync("app/components/group-invitations.tsx", "utf8");
  assert.match(invitationUi, /identity\.mode !== "authenticated"/);
