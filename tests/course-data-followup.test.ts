...
test('supplement import refuses wrong QA ref, keeps original evidence and never edits rounds',()=>{assert.throws(()=>execFileSync(process.execPath,['scripts/reviewed-card-supplement-sql.mjs','wrong'],{stdio:'pipe'}));const sql=execFileSync(process.execPath,['scripts/reviewed-card-supplement-sql.mjs','bymeopxkxapfizeeqeyb'],{encoding:'utf8'});assert.match(sql,/CARD_CONFLICT_REVIEW_REQUIRED/);assert.match(sql,/supplementOriginal/);assert.doesNotMatch(sql,/delete from|update public\.rounds|alter table/i);});
test('official card provenance and version frozen without auto applying category',()=>{const c={id:'c',clubId:'club',name:'Test',clubName:'Test',sourceUrl:'old',observedAt:'2026-09-20',dataVersion:'v1'} as ReviewedCatalogCourse;const t={id:'t',name:'Blue',yards:100,holes:[],nineRatings:[],course_rating:56,slope_rating:98,qa_status:'PASS',supplement:{sourceUrl:'https://club.invalid/card',authority:'Official card',observedAt:'2026-09-20',hash:'123456789abcdef'}} as unknown as ReviewedTeeSource;const snapshot=JSON.parse(JSON.stringify(reviewedTeeToCourse(c,t)));t.supplement!.sourceUrl='changed';assert.equal(snapshot.sourceUrl,'https://club.invalid/card');assert.equal(snapshot.dataVersion,'v1:card-123456789abc');assert.equal(snapshot.rating,undefined);});
