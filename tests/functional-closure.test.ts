...
test("Editar activa y navegar no borra scores; Par se confirma por gesto explícito",()=>{
  ...
  assert.match(app,/onClick=\{confirmSuggestedScores\}/);assert.match(nav,/window.history.back\(\)/);assert.match(nav,/previous.scroll/);
});
