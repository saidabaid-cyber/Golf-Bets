...
test("salida H10 liquida H18 antes de abrir la segunda vuelta en H1", () => {
  const order = playOrder(10);
  for (const kind of ["vipers", "camels", "fish"] as CounterBetKind[]) {
    const events: CounterBetEvent[] = [
      { id: `${kind}-18`, kind, hole: 18, playerId: "a", quantity: 1 },
    ...
    const result = calculateCounterBet(kind, players, counterConfig(), events, emptyCounterBetKeepers(), order, new Set(order.slice(0, 9)));
    assert.deepEqual(result.halves.map((half) => half.settled), [true, false]);
    assert.ok(result.halves[0].transfers.every((transfer) => transfer.metadata?.nine === "first_half"));
    assert.equal(result.halves[1].transfers.length, 0);
  }

  for (const kind of ["camels", "fish"] as CounterBetKind[]) {
    const events: CounterBetEvent[] = [
      { id: `${kind}-18-a`, kind, hole: 18, playerId: "a", quantity: 1 },
      { id: `${kind}-18-b`, kind, hole: 18, playerId: "b", quantity: 1 },
    ];
    const enabled = [{ kind, config: counterConfig() }];
    const keepers = emptyCounterBetKeepers();
    assert.match(requiredSideBetCapture(18, enabled, keepers, { enabled: false, participantIds: [] }, undefined, events, order), /Selecciona/);
    const selected = updateCounterBetKeeper(keepers, kind, "first_half", "a", order);
