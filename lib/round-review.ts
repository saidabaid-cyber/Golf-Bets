...
export function persistPendingRoundReview(
  ...
  const pending: Record<string, unknown> = { ...draft, reviewPending: true, lifecycleState: "completed" };
  ...
  if (!verified || verified.roundId !== pending.roundId || verified.reviewPending !== true || verified.lifecycleState !== "completed") {
