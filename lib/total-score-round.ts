...
import { normalizeRoundStartHole, playOrderForHoles } from "./engine";

export function totalScoreOrder(holes: 9 | 18, start: number, courseHoles = Array.from({ length: 18 }, (_, index) => index + 1)) {
  return playOrderForHoles(courseHoles, start).slice(0, holes);
}
export function validTotalOnly(round: RoundSnapshot, accountUserId: string): boolean {
  ...
  return Boolean(capture?.version === 1 && !capture.holesCompletedAt && Number.isInteger(capture.grossTotal)
    ...
    && JSON.stringify(round.order) === JSON.stringify(totalScoreOrder(round.roundHoles!, normalizeRoundStartHole(round.startHole), round.courseSnapshot.holes.map((hole) => hole.number))));
}
export function createTotalScoreRound(input: { id: string; course: Course; player: Player; date: string; holes: 9 | 18; start: number; total: number; now: string }): RoundSnapshot {
  ...
  const order = totalScoreOrder(holes, start, course.holes.map((hole) => hole.number));
