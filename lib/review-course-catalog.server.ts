...
export async function loadReviewedCourseCatalog():Promise<ReviewedCatalogCourse[]> {
  ...
  const [clubs,courses,tees]=await Promise.all([
    ...
    db.from('golf_courses').select('id,club_id,name,source_url,verified_at,catalog_metadata').eq('provider','OWNER_CATALOG_REVIEW').eq('active',true).limit(1000).abortSignal(AbortSignal.timeout(12000)),
  ...
  const data:ReviewedCatalogCourse[]=courses.data.flatMap(c=>{
    ...
    return [{id:c.id,clubId:club.id,name:c.name,clubName:club.name,city:club.city??undefined,stateRegion:club.state_region??undefined,
