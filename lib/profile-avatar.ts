...
export type ProfileAvatarType = "photo" | "emoji" | "custom_avatar" | "none";
...
export function profileAvatarType(value: string | null | undefined): ProfileAvatarType {
  if (!value?.trim()) return "none";
  if (isProfileEmojiAvatar(value)) return "emoji";
  if (parseManualAvatarUrl(value)) return "custom_avatar";
  // Legacy image-provider assets are ordinary secure photos, never rejected.
  return "photo";
}
