...
export function socialIdFromQr(value:string, currentOrigin:string): string | null {
 try { const url = new URL(value); if (![new URL(currentOrigin).origin, QA_SOCIAL_ORIGIN].includes(url.origin) || url.username || url.password || url.pathname !== "/" || url.hash || [...url.searchParams.keys()].some(k => k !== "friend")) return null;
