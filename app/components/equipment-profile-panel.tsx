...
export function EquipmentProfilePanel({ userId, accessToken, defaultHandicap, ballFitDefaults }: EquipmentProfilePanelProps) {
  ...
  return <div className={styles.stack}>
    ...
    {fitOpen && <div className={styles.editorBackdrop} role="presentation"><section ref={fitDialogRef} tabIndex={-1} className={styles.editorSheet} role="dialog" aria-modal="true" aria-label="The Backyard Ball Fit"><div className={styles.sheetHandle} />{ballCatalog.items.length ? <BallFitWizard userId={userId} defaultHandicap={defaultHandicap} profileDefaults={ballFitDefaults} currentBall={currentBall} catalog={ballCatalog.items} onCancel={() => setFitOpen(false)} onComplete={completeFit} /> : <div className={ballCatalog.status === "loading" ? styles.loadingState : styles.errorState} role="status">{ballCatalog.status === "loading" ? "Cargando catálogo de bolas…" : <>No pudimos cargar el catálogo. <button type="button" className="textButton" onClick={ballCatalog.retry}>Reintentar</button></>}</div>}</section></div>}
