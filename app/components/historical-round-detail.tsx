...
import { summarizePlayerAdvancedStats } from "../../lib/advanced-stats";
...
export function HistoricalRoundDetail({ round, onEdit, onPhoto }: { round: RoundSnapshot; onEdit: () => void; onPhoto: () => void }) {
  const owner = round.players?.find((player) => player.id === round.ownerId) || round.players?.find((player) => player.name === round.ownerName) || round.players?.[0];
  const advanced = owner ? summarizePlayerAdvancedStats(round.advancedStats, owner.id, round.order || []) : null;
  return <section className="card historicalDetail"><h1>{round.courseName}</h1><p>{round.date} · {round.ownerName} · {round.roundHoles || 18} hoyos · salida H{round.startHole || round.order?.[0] || 1}</p>
    ...
    {advanced && advanced.capturedHoles > 0 && <div className="historicalAdvancedSummary" aria-label="Estadísticas avanzadas guardadas"><b>Estadísticas capturadas de {owner?.name || "jugador principal"}</b><span>Fairways {advanced.fairwaysHit}/{advanced.fairwayAttempts} · GIR {advanced.greensInRegulation}/{advanced.greenAttempts} · Penalidades {advanced.penaltyStrokes}</span><small>{advanced.capturedHoles} hoyo{advanced.capturedHoles === 1 ? "" : "s"} con al menos un dato avanzado.</small></div>}
