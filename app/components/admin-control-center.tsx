...
export function AdminControlCenter() {
  ...
  const load = useCallback(async (nextView: View = view) => {
    ...
    try {
      const result = await api(`/api/admin/control-center?view=${nextView}&limit=100`);
  ...
  return <main className={styles.page}>
    ...
    <div className={styles.adminShell}>
      ...
      <section className={styles.workspace}>
        ...
        {view === "courses" && <CourseWorkspace data={data} loading={loading} request={api} submit={(payload) => mutate(payload, "Borrador de campo creado. Falta Review, Verify y Publish.", "revisions")} />}
        {view === "equipment" && <><EquipmentWorkspace data={data} loading={loading} request={api} submit={(payload) => mutate(payload, "Borrador de equipment creado sin publicarlo.", "revisions")} /><AssetEditor token={token} /></>}
...
function CourseWorkspace({ data, loading, request, submit }: { data: Json | null; loading: boolean; request: (path: string, options?: RequestInit) => Promise<Json>; submit: (payload: Json) => void }) {
  ...
  async function search() {
    ...
    try { const result = await request(`/api/admin/control-center?view=courses&limit=100&q=${encodeURIComponent(query)}`); setItems(list(result.items)); setStatus(""); }
  ...
  async function open(courseId: string) {
    ...
    try { const result = await request(`/api/admin/control-center?view=courses&courseId=${encodeURIComponent(courseId)}`); setSelected(object(result.item)); setStatus("Editas una nueva versión; lo publicado y los históricos no cambian hasta confirmar Publish."); }
...
function EquipmentWorkspace({ data, loading, request, submit }: { data: Json | null; loading: boolean; request: (path: string, options?: RequestInit) => Promise<Json>; submit: (payload: Json) => void }) {
...
function CourseOperations({ loading, submit, request, refresh, items }: { loading: boolean; submit: (payload: Json) => void; request: (path: string, options?: RequestInit) => Promise<Json>; refresh: () => Promise<void>; items: Json[] }) {
  ...
  return <>
    ...
    {items.length > 0 && <section className={styles.card}><h2>Configuraciones</h2>{workflowStatus && <div className={styles.notice}>{workflowStatus}</div>}<label>Si hay solapamiento<select value={overlapResolution} onChange={(event) => setOverlapResolution(event.target.value)}><option value="CANCEL">Cancelar publicación</option><option value="FINALIZE_PREVIOUS">Finalizar anterior</option><option value="SUPERSEDE_PREVIOUS">Superseder anterior</option></select></label><div className={styles.list}>{items.map((item) => <div className={styles.item} key={String(item.id)}><div><strong>{String(item.name)}</strong><span>{String(item.course_id)} · {String(item.status)} · v{String(item.version)}</span></div>{String(item.status) === "DRAFT" && <div className={styles.actions}><button type="button" onClick={() => void prepare(item)}>Preview / Diff</button>{previewHashes[String(item.id)] && <button type="button" className={styles.primary} onClick={() => void publish(item)}>Confirmar Publish</button>}</div>}</div>)}</div></section>}
  </>;
}

function ImportEditor({ loading, request }: { loading: boolean; request: (path: string, options?: RequestInit) => Promise<Json> }) {
  const csvTemplate = "id,brand,model,generation,year,category,usage,sourceName,sourceUrl\nsynthetic-test,Synthetic,Test Model,2026,2026,DRIVER,,Synthetic QA,https://example.invalid/source";
  const jsonTemplate = '[{"id":"synthetic-test","brand":"Synthetic","model":"Test Model","generation":"2026","year":2026,"category":"DRIVER","sourceName":"Synthetic QA","sourceUrl":"https://example.invalid/source"}]';
...
function RequestRow({ item, loading, onConvert }: { item: Json; loading: boolean; onConvert: (id: string, type: string) => void }) {
  ...
  return <article className={styles.item}><div><strong>{String(item.title || item.category)}</strong><span>{String(item.description)} · {String(item.request_status)}</span></div><div className={styles.actions}><select aria-label={`Tipo de Draft para ${String(item.title || item.category)}`} value={type} onChange={(event) => setType(event.target.value)}><option value="COURSE">Campo</option><option value="CLUB_EQUIPMENT">Bastón</option><option value="BALL">Bola</option><option value="SHAFT">Varilla</option><option value="LOCAL_RULE_SET">Regla local</option><option value="COURSE_CONFIGURATION">Cambio temporal</option><option value="REQUEST">Otro / soporte</option></select><button disabled={loading} onClick={() => onConvert(String(item.id), type)}>Crear borrador</button></div></article>;
}

function Quality({ data }: { data: Json | null }) {
  const courses = object(data?.courses) || {}; const equipment = object(data?.equipment) || {};
  return <><section className={styles.card}><h2>Calidad · Courses</h2><div className={styles.stats}>{Object.entries(courses).map(([key, value]) => <article className={styles.stat} key={key}><span>{key}</span><b>{String(value)}</b></article>)}</div></section><section className={styles.card}><h2>Calidad · Equipment</h2><div className={styles.stats}>{Object.entries(equipment).map(([key, value]) => <article className={styles.stat} key={key}><span>{key}</span><b>{String(value)}</b></article>)}</div><p className={styles.subtle}>Los huecos se reportan; no se completan por inferencia.</p></section></>;
}
