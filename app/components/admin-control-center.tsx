...
import { controlledImportTemplate, type AdminImportFormat, type AdminImportKind } from "../../lib/admin-imports";
...
function ImportEditor({ loading, request }: { loading: boolean; request: (path: string, options?: RequestInit) => Promise<Json> }) {
  const [kind, setKind] = useState<AdminImportKind>("COURSE");
  const [format, setFormat] = useState<AdminImportFormat>("CSV"); const [source, setSource] = useState(() => controlledImportTemplate("COURSE", "CSV")); const [preview, setPreview] = useState<Json | null>(null); const [status, setStatus] = useState(""); const [busy, setBusy] = useState(false);
  ...
  return <section className={styles.card}><div className={styles.sectionTitle}><div><h2>Importación controlada</h2><p>Upload → Parse → Normalize → Deduplicate → Validate → Diff. Este paso no publica.</p></div><span className={styles.badge}>NO AUTO-PUBLISH</span></div>{status && <div className={styles.notice}>{status}</div>}<form className={styles.form} onSubmit={save}><label>Catálogo<select name="kind" value={kind} onChange={(event) => { const next = event.target.value as AdminImportKind; setKind(next); setSource(controlledImportTemplate(next, format)); setPreview(null); setStatus("Plantilla actualizada para el catálogo seleccionado."); }}><option>COURSE</option><option>CLUB_EQUIPMENT</option><option>BALL</option><option>SHAFT</option></select></label><label>Formato<select value={format} onChange={(event) => { const next = event.target.value as AdminImportFormat; setFormat(next); setSource(controlledImportTemplate(kind, next)); setPreview(null); }}><option>CSV</option><option>JSON</option></select></label><label className={styles.wide}>Archivo CSV / JSON<input type="file" accept=".csv,.json,text/csv,application/json" onChange={(event) => void upload(event.target.files?.[0])} /></label><label className={styles.wide}>{format} de plantilla<textarea name="source" required value={source} onChange={(event) => setSource(event.target.value)} /></label><p className={`${styles.subtle} ${styles.wide}`}>La plantilla contiene únicamente el esquema del catálogo seleccionado. No incluye filas de ejemplo ni publica al validar; exporta XLSX a CSV antes de subir.</p><button className={styles.primary} disabled={loading || busy}>{busy ? "Validando…" : "Validar y mostrar Diff"}</button></form>{summary && <div className={styles.stats}>{Object.entries(summary).map(([key, value]) => <article className={styles.stat} key={key}><span>{key}</span><b>{String(value)}</b></article>)}</div>}{rows.length > 0 && <><div className={styles.tableWrap}><table className={styles.table}><thead><tr><th>Fila</th><th>Estado</th><th>Registro existente</th><th>Issues</th></tr></thead><tbody>{rows.map((row) => <tr key={String(row.row_number)}><td>{String(row.row_number)}</td><td><b>{String(row.status)}</b></td><td>{String(row.existing_entity_id || "—")}</td><td>{Array.isArray(row.issues) ? row.issues.join(" · ") : "—"}</td></tr>)}</tbody></table></div><button type="button" className={styles.primary} disabled={loading || busy || !rows.some((row) => ["NEW", "UPDATE"].includes(String(row.status)))} onClick={() => void confirm()}>Aprobar filas válidas y crear Drafts</button></>}</section>;
}
