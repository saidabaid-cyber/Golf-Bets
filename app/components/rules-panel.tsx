...
export function RulesPanel({
  ...
  const results = useMemo(() => searchGolfRules(query), [query]);
  ...
  return <>
    ...
    <section className="card" id="buscar-regla">
      ...
      <label>Buscar en las Reglas de Golf</label>
      <input type="search" value={query} placeholder="Ej. bola movida, camino, estaca roja…" onChange={(event) => setQuery(event.target.value)} />
      <div className="rulesResults">
        {!results.length && <div className="empty">No hay una coincidencia segura. Abre el reglamento oficial o pregunta al Comité.</div>}
        {results.slice(0, query ? 8 : 4).map((entry) => <article className="ruleResult" key={entry.id}>
          <div className="row between"><b>Regla {entry.rule}</b><a href={entry.sourceUrl} target="_blank" rel="noreferrer">Fuente oficial ↗</a></div>
          <h3>{entry.title}</h3>
    ...
    <section className="card" id="preguntar-ia">
      ...
      <form onSubmit={ask}>
        <label>Describe qué pasó</label>
        <textarea rows={4} maxLength={1200} value={question} placeholder="Mi bola está fuera del camino pero mis pies están sobre el camino…" onChange={(event) => setQuestion(event.target.value)} />
    ...
    <section className="card officialLinks" id="reglamento-usga">
      ...
      <div className="officialDocuments">
        ...
        <div className="officialDocumentGrid">{OFFICIAL_RULES_DOCUMENTS.map((document) => <article key={document.id}>
          ...
          <b className="sourceUsed">Usado por IA ✓</b>
          <div className="documentActions"><a className="primary" href={document.localUrl} target="_blank" rel="noreferrer">Abrir documento</a><a href={document.officialUrl} target="_blank" rel="noreferrer">Fuente oficial ↗</a></div>
        </article>)}</div>
      </div>
