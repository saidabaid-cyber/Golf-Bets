...
export function CounterBetHolePanel({ kind, config, players, events, hole, order, keepers, onQuantity, onDistance, onKeeper, resolutionOnly = false }: {
  ...
  return <section className="card compact sideEventCard">
    ...
    {asksKeeper && <label className="keeperSelect"><b>¿Quién se queda {meta.plural}?</b><span>Hubo empate en el último evento de H{latest.hole}. Selecciona quién lo generó al final.</span><select value={keepers[kind]?.[period] || (legacyNine ? keepers[kind]?.[legacyNine] : "") || ""} onChange={event => onKeeper(event.target.value, period)}><option value="">Seleccionar…</option>{tieCandidates.map(candidate => <option key={candidate.playerId} value={candidate.playerId}>{players.find(player => player.id === candidate.playerId)?.name || "Sin nombre"}</option>)}</select></label>}
  </section>;
}
