...
type EditDraft = Pick<BackyardProfileDetails, "givenName" | "familyName" | "username" | "handedness" | "homeClub" | "homeClubId" | "preferredTee" | "countryCode" | "country" | "stateCode" | "state">;
...
export function ProfileAccountPanel({ view, rootNavigationKey = 0, openAiPrivacySettings = false, onAiPrivacyOpened, history = [], indexControl, focusSection = "profile", highContrast, onHighContrastChange, notificationsEnabled, onNotificationsEnabledChange, golfInsights, statisticsResetAt, onStatisticsReset, onOpenStats, onOpenAccount, onOpenPrivacy, onOpenEquipment, onBackToProfile }: ProfileAccountPanelProps) {
  ...
  if (view === "profile" && identity.mode === "authenticated" && editing) return <>
    ...
    <section id="profile-edit-personal" className="card profileEditCard"><h2>Datos personales</h2><div className="profileEditGrid">
      ...
      <fieldset className="handednessChoice"><legend>Mano dominante</legend><label><input type="radio" name="profile-handedness" checked={draft.handedness === "right"} onChange={() => setDraft((current) => ({ ...current, handedness: "right" }))} />Derecha</label><label><input type="radio" name="profile-handedness" checked={draft.handedness === "left"} onChange={() => setDraft((current) => ({ ...current, handedness: "left" }))} />Izquierda</label></fieldset>
  ...
  return <>
    ...
    {view === "profile" && identity.mode === "authenticated" && <main className="profileMobileStack">
      ...
      <section className="card profileCompactCard"><div className="profileCompactHeading"><div><span>INFORMACIÓN DE GOLF</span><h2>Tu juego</h2></div><button type="button" className="textButton" onClick={() => setEditing(true)}>Editar</button></div><div className="profileCompactRows"><div><span>{indexLabel}</span><b>{profileHandicapLabel(selectedIndex.value)}</b></div><div><span>Home Club</span><b>{identity.homeClub || "Sin indicar"}</b></div><div><span>Tee habitual</span><b>{identity.preferredTee || "Sin indicar"}</b></div><div><span>Mano dominante</span><b>{identity.handedness === "right" ? "Derecha" : identity.handedness === "left" ? "Izquierda" : identity.handedness === "ambidextrous" ? "Ambidiestra" : "Sin indicar"}</b></div></div><HandicapSourceChoices control={indexControl} authenticated={identity.mode === "authenticated"} /></section>
    ...
    {view === "account" && <>
      ...
      <section className="card accountCompactCard"><h2>Preferencias</h2><label className="accountSettingRow"><span><b>Alto contraste</b><small>Preferencia visual</small></span><input type="checkbox" checked={highContrast} onChange={(event) => onHighContrastChange(event.target.checked)} /></label></section>
      <section className="card accountCompactCard"><h2>Notificaciones</h2><label className="accountSettingRow"><span><b>Avisos dentro de la app</b><small>Actividad nueva dentro de Social; no solicita permisos del dispositivo.</small></span><input type="checkbox" checked={notificationsEnabled} onChange={(event) => onNotificationsEnabledChange(event.target.checked)} aria-label="Activar avisos dentro de la app" /></label>{identity.accessToken && <SocialSharingPreferences accessToken={identity.accessToken} />}</section>
