...
export function CatalogCoursePicker({token,onSelect,onSelectClub,onSelectHomeCourse,selectedName='',onRequest,showHeading=true,purpose='round',onSelectionReadyChange}:{token?:string|null;onSelect?:(course:Course,cards:Course[])=>void;onSelectClub?:(club:{clubId:string;clubName:string})=>void;onSelectHomeCourse?:(selection:{clubId:string;clubName:string;courseId:string;courseName:string})=>void|Promise<void>;selectedName?:string;onRequest?:()=>void;showHeading?:boolean;purpose?:'round'|'home-club';onSelectionReadyChange?:(ready:boolean)=>void}) {
  ...
  const [club,setClub]=useState<string|null>(null),[cards,setCards]=useState<Course[]>([]),[chosen,setChosen]=useState(''),[retry,setRetry]=useState(0),[retryCourseId,setRetryCourseId]=useState<string|null>(null);
  ...
  async function selectCourse(id:string) {
    if(!id)return;
    const sequence=++loadSequence.current;setLoading(true);setError('');setRetryCourseId(null);setCards([]);setChosen('');
  ...
  function selectClub(entry:Entry) {++loadSequence.current;setLoading(false);setError('');setClub(entry.clubId);setQuery('');setCards([]);setChosen('');setRetryCourseId(null);if(purpose==='home-club')onSelectionReadyChange?.(false);onSelectClub?.(entry);const layouts=entries.filter(c=>c.clubId===entry.clubId);if(layouts.length===1)void selectCourse(layouts[0].id);}
  function locate() {
    cancelLocation.current();
    setLocation({status:'loading'});
    cancelLocation.current=requestCourseLocation(navigator.geolocation,setLocation);
  }
