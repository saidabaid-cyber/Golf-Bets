...
export function DevicePermissions({ kind = 'all' }: { kind?: 'all' | 'location' | 'notifications' }) {
  ...
  useEffect(() => {
    ...
    const refresh = async () => {
      ...
      } catch { if (alive) setLocation('unknown'); }
    };
  ...
  const label = (state: string) => ({ granted: 'Permitido', denied: 'Bloqueado en este navegador', prompt: 'Sin solicitar', default: 'Sin solicitar', unknown: 'Este navegador no permite consultar el estado. Puedes revisarlo en los ajustes del sitio.', unsupported: 'No disponible en este navegador', loading: 'Solicitando permiso…', consultando: 'Consultando…' })[state] || 'No disponible';
  return <div className="devicePermissions">
    {kind !== 'notifications' && <section><h3>Ubicación</h3><p>Para encontrar campos cercanos. No guardamos ni enviamos tu ubicación.</p><p role="status">{label(location)}</p><button type="button" className="secondary" disabled={location === 'loading' || location === 'unsupported'} onClick={() => {
      ...
      cancel.current = requestCourseLocation(navigator.geolocation, result => { setLocation(result.status === 'located' ? 'granted' : result.status === 'denied' ? 'denied' : result.status === 'unsupported' ? 'unsupported' : 'unknown'); if (result.status !== 'located') setMessage('No se obtuvo ubicación. Puedes buscar campos manualmente y administrar el permiso en los ajustes del sitio.'); });
