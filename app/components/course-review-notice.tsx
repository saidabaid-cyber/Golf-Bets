...
export function CourseReviewNotice({course,roundHoles,startHole}:{course:Course;roundHoles:9|18;startHole:1|10}) {
  const review=course.catalogReview;if(!review)return null;
  const nine=review.nineRatings.find(n=>n.segment===(startHole===1?'FRONT':'BACK'));
  return <aside className="notice"><b>{course.teeName} · {roundHoles===9?`Hoyos ${startHole===1?'1–9':'10–18'}`:'18 hoyos'}</b>
