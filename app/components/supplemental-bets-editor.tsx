...
function ParticipantChips({ players, selected, onChange }: { players: Player[]; selected: string[] | undefined; onChange: (ids: string[]) => void }) {
...
function ItemShell({ bet, label, onToggle, onRemove, children, locked, detailsOnly = false }: { bet: SupplementalBet; label: string; onToggle: () => void; onRemove: () => void; children: ReactNode; locked: boolean; detailsOnly?: boolean }) {
  ...
  return <article data-supplemental-editor={bet.id} className={`${styles.betItem} ${!bet.enabled ? styles.disabled : ""}`}>
    <div className={styles.itemHeader}>
      ...
      {!detailsOnly && <span className={styles.itemActions}><Switch on={bet.enabled} label={label} disabled={locked} onChange={onToggle} /></span>}
      <button type="button" className="remove" disabled={locked} aria-label={`Eliminar ${label}`} onClick={onRemove}>{detailsOnly ? "Quitar esta apuesta" : "×"}</button>
    </div>
...
export function SupplementalBetsEditor({ bets, players, onChange, requestActivation, locked = false, types = ORDER, roundHoles = 18, allowAdd = true, initiallyExpandActive = false, detailsOnly = false, assignmentMode = "round" }: { bets: SupplementalBet[]; players: Player[]; onChange: Dispatch<SetStateAction<SupplementalBet[]>>; requestActivation?: () => Promise<boolean>; locked?: boolean; types?: SupplementalBet["type"][]; roundHoles?: 9 | 18; allowAdd?: boolean; initiallyExpandActive?: boolean; detailsOnly?: boolean; assignmentMode?: "round" | "template" }) {
  ...
  const addNow = (type: SupplementalBet["type"]) => {
    ...
    onChange((current) => current.some((bet) => bet.id === id) ? current : [...current, createSupplementalBet(type, assignmentMode === "template" ? [] : players, id, roundHoles)]);
  ...
  const setTypeEnabledNow = (type: SupplementalBet["type"], enabled: boolean) => {
    ...
    if (enabled && !bets.some((bet) => bet.type === type)) {
      ...
      onChange((current) => current.some((bet) => bet.type === type)
        ...
        : [...current, createSupplementalBet(type, assignmentMode === "template" ? [] : players, id, roundHoles)]);
      switchRefs.current[type]?.focus({ preventScroll: true });
    } else {
  ...
  return <div ref={editorRef} className={`${styles.editor} ${groupLayout ? styles.groupEditor : ""}`.trim()}>{types.map((type) => {
    ...
    if (detailsOnly && !modeEnabled) return null;
    return <ResultAccordion key={type} id={`setup-${type}`} title={title} open={modeEnabled && Boolean(openTypes[type])} disclosureDisabled={!modeEnabled || locked} onOpenChange={(open) => { if (modeEnabled && !locked) setOpenTypes((current) => ({ ...current, [type]: open })); }} className={`setupBetsAccordion ${groupLayout ? styles.groupModeCard : ""}`.trim()} headerAction={<span className={styles.headerActions}><BetHelpButton kind={type} />{!detailsOnly && <Switch buttonRef={(node) => { switchRefs.current[type] = node; }} on={modeEnabled} label={label} disabled={locked} onChange={() => setTypeEnabled(type, !modeEnabled)} />}</span>}>
      {modeEnabled && <>
      {allowAdd && <div className={styles.modeTools}><button type="button" className="textButton" onClick={() => add(type)}>+ Agregar</button></div>}
      {typeBets.map(({ bet, index }) => <ItemShell key={bet.id} bet={bet} locked={locked} detailsOnly={detailsOnly} label={`${SUPPLEMENTAL_BET_LABELS[type]} ${index + 1}`} onToggle={() => bet.enabled ? update(bet.id, { enabled: false, enabledBeforeCategoryOff: undefined }) : runAfterConsent(() => update(bet.id, { enabled: true, enabledBeforeCategoryOff: undefined }))} onRemove={() => detailsOnly ? update(bet.id, { enabled: false }) : remove(bet.id)}>
        ...
        {bet.type === "individual_pressures" && <>
          ...
          {assignmentMode === "round" && <><label className="miniLabel">Participan</label><ParticipantChips players={players} selected={bet.participantIds} onChange={(participantIds) => update(bet.id, { participantIds })} /></>}
        </>}
        {bet.type === "team_pressures" && <>
          <div className="grid3"><MoneyField label="Valor por presión" value={bet.value} onChange={(value) => update(bet.id, { value })} /><HcpPercentageInput value={bet.hcpPct} disabled={locked} onChange={(hcpPct) => update(bet.id, { hcpPct })} /><label>Comparación<select value={bet.metric} onChange={(event) => update(bet.id, { metric: event.target.value as typeof bet.metric })}><option value="low">Low Ball</option><option value="high">High Ball</option><option value="low_high">Low + High</option></select></label></div>
          <div className="grid3"><label>Modalidad<select value={bet.virtualMode} onChange={(event) => update(bet.id, { virtualMode: event.target.value as typeof bet.virtualMode })}><option value="standard">2 vs 2</option><option value="mudo">Mudo · siempre Par</option><option value="yoyo">Yo-Yo · copia pareja</option></select></label><label className="checkRow"><input type="checkbox" checked={bet.carryEnabled} onChange={(event) => update(bet.id, { carryEnabled: event.target.checked })} />Carry</label><NumberField label="Score máximo (solo apuesta)" value={bet.abandonedMaxScore} min={1} onChange={(abandonedMaxScore) => update(bet.id, { abandonedMaxScore })} /></div>
          {assignmentMode === "round" && <><label className="miniLabel">Participan</label><ParticipantChips players={players} selected={bet.participantIds} onChange={(participantIds) => update(bet.id, { participantIds })} /><label className="miniLabel">Jugadores retirados · el máximo solo aplica a esta apuesta; la tarjeta sigue incompleta</label><ParticipantChips players={players.filter((player) => (Array.isArray(bet.participantIds) ? bet.participantIds : []).includes(player.id))} selected={bet.abandonedPlayerIds || []} onChange={(abandonedPlayerIds) => update(bet.id, { abandonedPlayerIds })} />{bet.virtualMode === "standard" && <><label className="miniLabel">Equipo A · el resto forma Equipo B</label><ParticipantChips players={players.filter((player) => (Array.isArray(bet.participantIds) ? bet.participantIds : []).includes(player.id))} selected={bet.teamA} onChange={(teamA) => update(bet.id, { teamA: teamA.slice(-2) })} /></>}</>}
        </>}
        {bet.type === "chicago" && <>
          <div className="grid3"><NumberField label="Base de cuota" value={bet.quotaBase} onChange={(quotaBase) => update(bet.id, { quotaBase })} /><HcpPercentageInput value={bet.hcpPct ?? 100} disabled={locked} onChange={(hcpPct) => update(bet.id, { hcpPct })} /><MoneyField label="Valor por punto" value={bet.valuePerPoint} onChange={(valuePerPoint) => update(bet.id, { valuePerPoint })} /></div>
          <div className="grid4"><NumberField label="Birdie o mejor" value={bet.points?.birdieOrBetter ?? 4} onChange={(value) => update(bet.id, { points: Object.assign({ birdieOrBetter: 4, par: 2, bogey: 1, doubleBogeyOrWorse: 0 }, bet.points || {}, { birdieOrBetter: value }) })} /><NumberField label="Par" value={bet.points?.par ?? 2} onChange={(value) => update(bet.id, { points: Object.assign({ birdieOrBetter: 4, par: 2, bogey: 1, doubleBogeyOrWorse: 0 }, bet.points || {}, { par: value }) })} /><NumberField label="Bogey" value={bet.points?.bogey ?? 1} onChange={(value) => update(bet.id, { points: Object.assign({ birdieOrBetter: 4, par: 2, bogey: 1, doubleBogeyOrWorse: 0 }, bet.points || {}, { bogey: value }) })} /><NumberField label="Doble o peor" value={bet.points?.doubleBogeyOrWorse ?? 0} onChange={(value) => update(bet.id, { points: Object.assign({ birdieOrBetter: 4, par: 2, bogey: 1, doubleBogeyOrWorse: 0 }, bet.points || {}, { doubleBogeyOrWorse: value }) })} /></div>
          {assignmentMode === "round" && <><label className="miniLabel">Participan</label><ParticipantChips players={players} selected={bet.participantIds} onChange={(participantIds) => update(bet.id, { participantIds })} /></>}
        </>}
        {bet.type === "vegas" && <>
          <div className="grid3"><MoneyField label="Valor por unidad" value={bet.valuePerUnit} onChange={(valuePerUnit) => update(bet.id, { valuePerUnit })} /><HcpPercentageInput value={bet.hcpPct} disabled={locked} onChange={(hcpPct) => update(bet.id, { hcpPct })} /><label>Rotación<select value={bet.rotation} onChange={(event) => update(bet.id, { rotation: event.target.value as typeof bet.rotation })}><option value="fixed">Parejas fijas</option><option value="each_hole">Cada hoyo</option><option value="blocks">Por bloques</option></select></label></div>
          ...
          {assignmentMode === "round" && <><label className="miniLabel">Participan · exactamente 4</label><ParticipantChips players={players} selected={bet.participantIds} onChange={(participantIds) => update(bet.id, { participantIds: participantIds.slice(-4) })} /><label className="miniLabel">Equipo A · el resto forma Equipo B</label><ParticipantChips players={players.filter((player) => (Array.isArray(bet.participantIds) ? bet.participantIds : []).includes(player.id))} selected={bet.teamA} onChange={(teamA) => update(bet.id, { teamA: teamA.slice(-2) })} /></>}
        </>}
