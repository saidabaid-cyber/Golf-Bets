    ...
    return <ResultAccordion key={type} id={`setup-${type}`} title={title} open={modeEnabled && Boolean(openTypes[type])} disclosureDisabled={!modeEnabled || locked} onOpenChange={(open) => { if (modeEnabled && !locked) setOpenTypes((current) => ({ ...current, [type]: open })); }} className={`setupBetsAccordion ${groupLayout ? styles.groupModeCard : ""}`.trim()} headerAction={<span className={styles.headerActions}><BetHelpButton kind={type} />{!detailsOnly && <Switch buttonRef={(node) => { switchRefs.current[type] = node; }} on={modeEnabled} label={label} disabled={locked} onChange={() => setTypeEnabled(type, !modeEnabled)} />}</span>}>
      ...
      {typeBets.map(({ bet, index }) => <ItemShell key={bet.id} bet={bet} locked={locked} detailsOnly={detailsOnly} label={`${SUPPLEMENTAL_BET_LABELS[type]} ${index + 1}`} onToggle={() => bet.enabled ? update(bet.id, { enabled: false, enabledBeforeCategoryOff: undefined }) : runAfterConsent(() => update(bet.id, { enabled: true, enabledBeforeCategoryOff: undefined }))} onRemove={() => detailsOnly ? update(bet.id, { enabled: false }) : remove(bet.id)}>
        ...
        {bet.type === "team_pressures" && <>
          ...
          <label className="miniLabel">Participan</label><ParticipantChips players={players} selected={bet.participantIds} onChange={(participantIds) => update(bet.id, { participantIds })} />
          <label className="miniLabel">Jugadores retirados · el máximo solo aplica a esta apuesta; la tarjeta sigue incompleta</label><ParticipantChips players={players.filter((player) => (Array.isArray(bet.participantIds) ? bet.participantIds : []).includes(player.id))} selected={bet.abandonedPlayerIds || []} onChange={(abandonedPlayerIds) => update(bet.id, { abandonedPlayerIds })} />
          {bet.virtualMode === "standard" && <><label className="miniLabel">Equipo A · el resto forma Equipo B</label><ParticipantChips players={players.filter((player) => (Array.isArray(bet.participantIds) ? bet.participantIds : []).includes(player.id))} selected={bet.teamA} onChange={(teamA) => update(bet.id, { teamA: teamA.slice(-2) })} /></>}
        </>}
        {bet.type === "chicago" && <>
        ...
        {bet.type === "vegas" && <>
          ...
          <label className="miniLabel">Equipo A · el resto forma Equipo B</label><ParticipantChips players={players.filter((player) => (Array.isArray(bet.participantIds) ? bet.participantIds : []).includes(player.id))} selected={bet.teamA} onChange={(teamA) => update(bet.id, { teamA: teamA.slice(-2) })} />
        </>}
