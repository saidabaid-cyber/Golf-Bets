...
export function ProfileCompletionRing({ token, avatar, name, revision, onOpen }: { token?: string | null; avatar: string; name: string; revision: string; onOpen: (section: CompletionSection) => void }) {
 ...
 return <><button type="button" className={styles.ring} style={{ "--progress": `${result?.progress.percent ?? 0}%`, ...(result?.progress.percent === 100 ? { background: "transparent" } : {}) } as CSSProperties} onClick={() => { setOpen(true); void refresh(); }} aria-label={`Perfil ${result ? result.progress.percent + "% completado" : "consultar progreso"}`}><span><ProfileAvatarMedia value={avatar} fallback={name[0] || "J"} /></span>{result?.progress.percent !== 100 && <b>{result ? `${result.progress.percent}%` : "—"}</b>}</button>
