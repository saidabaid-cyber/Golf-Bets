...
export function ProfileCompletionRing({ token, avatar, name, revision, onOpen }: { token?: string | null; avatar: string; name: string; revision: string; onOpen: (section: CompletionSection) => void }) {
 ...
 {open && <div className="modalBackdrop" onKeyDown={e => { if(e.key === "Escape" && !busy) setOpen(false); }}><section className={`confirmDialog ${styles.dialog}`} role="dialog" aria-modal="true" aria-labelledby="profile-completion-title"><button autoFocus type="button" className="textButton" disabled={busy} onClick={() => setOpen(false)}>Cerrar ×</button><h2 id="profile-completion-title">Tu perfil {result ? `${result.progress.percent}%` : ""}</h2><p>Siete secciones con el mismo peso. No necesitas GHIN, publicar tu perfil ni aceptar permisos opcionales. Esto no bloquea jugar.</p>
 ...
 {error && <p role="alert">{error}</p>}<button type="button" className="primary" disabled={busy || !result} onClick={() => void save()}>{busy ? "Guardando…" : "Guardar respuestas"}</button>{!result && <button type="button" onClick={() => void refresh()}>Reintentar</button>}</section></div>}</>;
}
