  ...
  return <div className={styles.details}>
    ...
    {groupTemplateCoreDefinitions().filter((item) => Boolean(coreConfig(value, item.templateEditor.key)?.enabled)).map((item) => {
      ...
      return <details className={styles.detailCard} key={item.id}><summary><span>{item.icon} {item.label}</span><small>{item.description} · Editar</small></summary><fieldset disabled={locked}>
        ...
        {"value" in config && <Field label="Valor" value={config.value} step={0.01} onChange={(next) => updateCore(key, { value: next })} />}
        {"hcpPct" in config && <Field label="HCP %" value={config.hcpPct} min={0} max={100} onChange={(next) => updateCore(key, { hcpPct: next })} />}
        {"decimals" in config && <label className={styles.field}>Redondeo HCP<select value={String(config.decimals || "decimal")} onChange={(event) => updateCore(key, { decimals: event.target.value })}><option value="decimal">Mantener decimales</option><option value="half_up">.5 hacia arriba</option><option value="half_down">.5 hacia abajo</option><option value="six_up">.6 hacia arriba</option><option value="four_down">.4 hacia abajo</option><option value="round">Redondeo normal</option></select></label>}
        ...
        {key === "foursome" && <><div className={styles.fieldsRow}><label className={styles.field}>Modalidad<select value={value.betConfig.foursome.mode} onChange={(event) => updateCore(key, { mode: event.target.value })}><option value="fixed">Fijo</option><option value="fixed_points">Fijo + Patada</option><option value="points">Solo puntos / patada</option></select></label>{(value.betConfig.foursome.mode === "fixed" || value.betConfig.foursome.mode === "fixed_points") && <Field label="Valor fijo" value={value.betConfig.foursome.fixedValue} step={0.01} onChange={(fixedValue) => updateCore(key, { fixedValue })} />}{(value.betConfig.foursome.mode === "points" || value.betConfig.foursome.mode === "fixed_points") && <Field label="Valor por punto / patada" value={value.betConfig.foursome.pointValue} step={0.01} onChange={(pointValue) => updateCore(key, { pointValue })} />}<label className={styles.field}>Tramos<select value={value.betConfig.foursome.segmentSize} onChange={(event) => setFoursomeSegmentSize(Number(event.target.value) as 3 | 6 | 9 | 18)}><option value="3">3 hoyos</option><option value="6">6 hoyos</option><option value="9">9 hoyos</option><option value="18">18 hoyos</option></select></label><Field label="Máximo en pareja base" value={foursomeMaxBaseAppearances} min={1} max={18} onChange={(next) => setFoursomeMaxBaseAppearances(Math.max(1, Math.trunc(next)))} />{value.roundDefaults.roundHoles === 18 && <label className={styles.field}>Presión · segunda vuelta<select value={value.betConfig.foursome.pressureMultiplier ?? (value.betConfig.foursome.pressSecond9 ? 2 : 1)} onChange={(event) => updateCore(key, { pressureMultiplier: Number(event.target.value), pressSecond9: Number(event.target.value) > 1 })}><option value="1">Sin presión</option><option value="2">2x</option><option value="3">3x</option><option value="4">4x</option><option value="5">5x</option></select></label>}</div><button type="button" className="secondary" onClick={generateFoursomes}>Generar foursomes automáticamente</button>{foursomeMessage && <small className={styles.editorMessage} role="status">{foursomeMessage}</small>}<div className={styles.segmentList}>{value.foursomeSegments.map((segment) => <article key={segment.id}><div><b>H{roundOrder[segment.startIndex]}–{roundOrder[segment.endIndex]}</b><small>{segment.generatedByBackyard ? "Generada por Backyard" : "Editada manualmente"}</small></div><Participants players={players.filter((player) => value.betConfig.foursome.participantIds.includes(player.id))} selected={segment.basePair} onChange={(basePair) => onChange((current) => ({ ...current, foursomeSegments: current.foursomeSegments.map((segmentItem) => segmentItem.id === segment.id ? markFoursomeSegmentEdited({ ...segmentItem, basePair: basePair.slice(-2) }) : segmentItem) }))} /></article>)}</div></>}
