...
export function PersonalQr({userId,name,username,avatar,onClose}:{userId:string;name:string;username:string;avatar:string;onClose:()=>void}){
 ...
 async function copyLink(){try{await navigator.clipboard.writeText(link);setMessage("Enlace copiado.");}catch{setMessage("No pudimos copiar el enlace. Intenta Compartir enlace.");}}
 return <section className={`card ${styles.panel} ${styles.identityCard}`}><button type="button" className="textButton" onClick={onClose}>← Social</button><header className={styles.heading}><span>GOOD GOLF, BETTER FRIENDS</span><h2>Mi QR</h2><p>Tu tarjeta. Tu comunidad.</p></header><div className={styles.identity}><div ref={avatarElement} className={styles.avatar}><ProfileAvatarMedia value={avatar} fallback={name[0]||"J"}/></div><div><b>{name}</b><p>{username?`@${username}`:"Configura tu username desde Perfil"}</p></div></div><canvas ref={canvas} className={styles.qr} aria-label={`Tarjeta QR de ${name}`}/><div className={styles.actions}><button type="button" className="primary" disabled={!ready} onClick={()=>void share(false)}>Compartir enlace</button><button type="button" className="secondary" disabled={!ready} onClick={()=>void share(true)}>Guardar / compartir imagen</button></div><button type="button" className="textButton" disabled={!ready} onClick={() => void copyLink()}>Copiar enlace de perfil</button><p className="hint">Comparte tu perfil, no tus datos privados. Tú decides a quién agregar.</p>{message&&<p role="status">{message}</p>}</section>;
}
export function SocialQrScanner({onFound,onClose,backLabel="Social"}:{onFound:(id:string)=>void;onClose:()=>void;backLabel?:string}){
