...
import { isProfileEmojiAvatar, PROFILE_EMOJI_AVATARS } from "../../lib/profile-avatar";
...
export function ProfileImagePicker({ value, onChange, kind = "profile" }: { value: string; onChange: (value: string) => void; kind?: "profile" | "group" }) {
  ...
  const [mode, setMode] = useState<"photo" | "avatar" | "emoji">(isProfileEmojiAvatar(value) ? "emoji" : value.startsWith("/avatars/") ? "avatar" : "photo");
  ...
  return <div className={styles.picker}>
    ...
    {kind === "profile" && <div className={styles.tabs} role="tablist" aria-label="Tipo de imagen"><button type="button" role="tab" aria-selected={mode === "photo"} data-active={mode === "photo"} onClick={() => setMode("photo")}>Foto</button><button type="button" role="tab" aria-selected={mode === "avatar"} data-active={mode === "avatar"} onClick={() => setMode("avatar")}>Avatares</button><button type="button" role="tab" aria-selected={mode === "emoji"} data-active={mode === "emoji"} onClick={() => setMode("emoji")}>Emoji</button></div>}
    ...
    {kind === "profile" && mode === "avatar" && <div className={styles.presets} aria-label="Avatares disponibles">{AVATARS.map((avatar) => {
    ...
    {kind === "profile" && mode === "emoji" && <div className={styles.emojis} aria-label="Emojis disponibles">{PROFILE_EMOJI_AVATARS.map((emoji) => <button type="button" key={emoji} data-active={value === emoji} aria-pressed={value === emoji} aria-label={`Elegir ${emoji}`} onClick={() => { onChange(emoji); setMessage(""); }}>{emoji}<span aria-hidden="true">{value === emoji ? "✓" : ""}</span></button>)}</div>}
