...
export function InitialDevicePermissions({ userId, onContinue }: { userId: string; onContinue: () => void }) {
  ...
  return <div className="devicePermissionChoices">
    <article><div><b>Ubicación</b><span>{statusLabel(value.location)}</span><small>Se usa una ubicación aproximada para ordenar campos cercanos. Es opcional.</small></div>{value.location === "granted" ? <strong>✓</strong> : <button type="button" className="secondary" disabled={busy !== null} onClick={() => void location()}>{busy === "location" ? "Solicitando…" : "Permitir ubicación"}</button>}</article>
...
export function DevicePermissionSettings({ userId }: { userId: string }) {
  const [value, setValue] = useState(() => typeof window === "undefined" ? emptyDevicePermissionPreferences(userId) : readDevicePermissionPreferences(localStorage, userId));
  useEffect(() => { void refreshDevicePermissionStateWithoutPrompt(localStorage, userId).then(setValue); }, [userId]);
  return <section className="card accountCompactCard"><h2>Permisos del dispositivo</h2>
    ...
    <p className="hint">Aquí puedes dejar de usar un permiso. Para volver a conceder uno bloqueado, hazlo desde los permisos de The Backyard en tu dispositivo.</p>
    <div className="accountInlineActions">{value.locationEnabled && <button type="button" className="secondary" onClick={() => setValue(disableLocationForApp(localStorage, userId))}>Desactivar ubicación</button>}{value.notificationsEnabled && <button type="button" className="secondary" onClick={() => setValue(disableNotificationsForApp(localStorage, userId))}>Desactivar notificaciones</button>}</div>
  </section>;
}
