...
export function PersonalCompact({ results, owner, name, hole, onOpen }: { results: Result[]; owner: string; name: (id: string) => string; hole?: number; onOpen: (id: string) => void }) {
  ...
  return <section className="card personalCompact"><h2>Personales</h2>{results.map(result => {
    ...
    return <button className="personalCompactCard" key={result.betId} onClick={() => onOpen(result.betId)}>
      ...
      {hole ? <div>{active.filter(item => !item.key.endsWith("18") || !active.some(other => other.kind === item.kind && !other.key.endsWith("18"))).map(component => {
        ...
        return <span key={component.key}>{component.kind === "match" ? "Match" : "Medal"}{component.key.endsWith("18") ? " 18" : ""}: {component.playedHoles === 0 ? "Esperando scores" : value === 0 ? "AS" : `${value > 0 ? owner : rival} ${Math.abs(value)} ${component.kind === "match" ? "UP" : Math.abs(value) === 1 ? "golpe arriba" : "golpes arriba"}`}</span>;
      ...
      <strong>{complete ? "Resultado" : "Provisional"}: {owner} {money(complete ? result.totalMoney : provisional)}</strong><small>Ver detalle →</small>
    </button>;
  })}</section>;
}
