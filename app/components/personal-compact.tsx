...
export function PersonalCompact({ results, owner, name, hole, title = "Nassau Individual", embedded = false, onOpen }: { results: Result[]; owner: string; name: (id: string) => string; hole?: number; title?: string; embedded?: boolean; onOpen: (id: string) => void }) {
  ...
  })}{!hole && <div className="personalCompactTotal"><span>Total Nassau Individual</span><strong className={ownerTotal > 0 ? "good" : ownerTotal < 0 ? "bad" : ""}>{owner}: {money(ownerTotal)}</strong></div>}</section>;
}
