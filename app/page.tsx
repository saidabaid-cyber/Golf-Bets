...
function normalizePolla(raw: any, ids: string[], startHole: number, roundHoles: number): BetConfig["polla"] {
  ...
  if (raw?.first9 || raw?.second9 || raw?.total18) {
    const first9 = normalizeComponent(raw.first9, defaults.first9);
    const second9 = normalizeComponent(raw.second9, defaults.second9);
    const migratePhysicalNines = raw.first9?.playedHalfVersion !== 1 && startHole === 10;
    return {
      first9: { ...(migratePhysicalNines ? second9 : first9), playedHalfVersion: 1 },
      second9: migratePhysicalNines && roundHoles >= 18 ? first9 : second9,
  ...
  return {
    first9: { ...defaults.first9, ...legacyBase, playedHalfVersion: 1, value: raw?.first9Value ?? raw?.nineValue ?? defaults.first9.value },
...
function GolfBetsApp() {
      ...
      if (draft) {
        ...
        if (draft.bets) {
          ...
          setBets({
            ...
            polla: normalizePolla(draft.bets.polla, draftPlayerIds, draft.startHole === 10 ? 10 : 1, draft.roundHoles === 9 ? 9 : 18),
  ...
  useEffect(() => {
    ...
    const persist = () => {
      ...
      try {
        const draft = { version: 8, course, courseSelected, startHole, roundHoles, handicapBasis: roundHandicapBasis, players, ownerId, bets, segments, personalBets, supplementalBets, manualBets, scores, scoreEdits, putts, unitEvents, counterBetEvents, counterBetKeepers, lobaHoles, ballFriendSetup, expenses, roundId, roundDate, currentIndex, reviewPending: roundReviewPending };
  ...
  }, [hydrated, identity.userId, identity.mode, identity.defaultHandicap, cloudLinked, courses, history, savedPersonalRivals, frequentPlayers, frequentGroups, highContrast, roundClosed, roundReviewPending, course, courseSelected, startHole, roundHoles, roundHandicapBasis, players, ownerId, bets, segments, personalBets, supplementalBets, manualBets, scores, scoreEdits, putts, unitEvents, counterBetEvents, counterBetKeepers, lobaHoles, ballFriendSetup, expenses, roundId, roundDate, currentIndex]);
  ...
  function roundDraftPayload(overrides: { scores?: Record<number, HoleScore>; scoreEdits?: ScoreRows; bets?: BetConfig; currentIndex?: number; reviewPending?: boolean } = {}) {
    return {
      version: 8, course, courseSelected, startHole, roundHoles, handicapBasis: roundHandicapBasis, players, ownerId,
  ...
  function persistReviewBeforeLeavingRound(savedScores: Record<number, HoleScore>, savedEdits: ScoreRows, savedBets: BetConfig, savedIndex: number) {
    try {
      ...
      trackLocalCloudCheckpoint(localStorage, draft, { highContrast, language: "es-MX", notificationsEnabled: false, defaultHandicap: identity.defaultHandicap }, previousDraft);
  ...
  function saveCourseDraft() {
    ...
    const apply = () => { setCourses((cs) => [saved, ...cs.filter((c) => c.id !== saved.id)]); setCourse(saved); setCourseSelected(true); setCourseSelectionError(false); goBack(); };
  ...
  function persistCommittedHoleBeforeAdvance(savedScores: Record<number, HoleScore>, savedEdits: ScoreRows, savedBets: BetConfig, savedIndex: number) {
    try {
      const previousDraft = readStoredJson<unknown>(localStorage, STORAGE_KEYS.draft, null);
      const draft = roundDraftPayload({ scores: savedScores, scoreEdits: savedEdits, bets: savedBets, currentIndex: savedIndex, reviewPending: false });
      ...
      persistRoundDraftCheckpoint(localStorage, draft);
      trackLocalCloudCheckpoint(localStorage, draft, { highContrast, language: "es-MX", notificationsEnabled: false, defaultHandicap: identity.defaultHandicap }, previousDraft);
      setDraftAvailable(true);
      setSaveStatus("saved");
      const offline = collectLocalCloudData(localStorage, identity.defaultHandicap, hadLocalPreferences.current);
      ...
      setSaveStatus("error");
      setFeedback("No se pudo guardar el hoyo en este dispositivo. Conservamos la captura en pantalla; libera espacio y vuelve a pulsar Guardar y siguiente hoyo.");
      return false;
    }
  }
  ...
  function saveAndAdvance() {
    ...
    const checkpointPersisted = savedIndex === order.length - 1
      ? persistReviewBeforeLeavingRound(committed.scores, committed.edits, savedBets, savedIndex)
      : persistCommittedHoleBeforeAdvance(committed.scores, committed.edits, savedBets, savedIndex);
      ...
      <section className="card">
        ...
        <div className="grid2">
          <div className={`courseSelectionField ${courseSelectionError ? "isMissing" : ""}`}><label htmlFor="round-course">Campo</label><select id="round-course" value={courseSelected ? course.name : ""} aria-invalid={courseSelectionError} aria-describedby={courseSelectionError ? "round-course-error" : undefined} onChange={(e) => {
            const next = courses.find((x) => x.name === e.target.value); if (next) confirmRoundChange("Cambiar campo modifica el Par/SI aplicado a los scores existentes.", () => { setCourse(next); setCourseSelected(true); setCourseSelectionError(false); });
