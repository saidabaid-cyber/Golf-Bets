...
// Frequent-player templates do not carry a verified username or privacy level.
const EMPTY_SOCIAL_DIRECTORY: SocialProfile[] = [];
...
function GolfBetsApp() {
  ...
  useEffect(() => {
    if (tab !== "setup") return;
    const frame = window.requestAnimationFrame(() => document.getElementById("round-course")?.scrollIntoView({ block: "start", behavior: "auto" }));
    return () => window.cancelAnimationFrame(frame);
  }, [courseSetupStage, tab]);
  ...
  const undoStack = useRef<Array<{ scores: Record<number, HoleScore>; scoreEdits: ScoreRows; putts: PuttsByHole; advancedStats: AdvancedStatsByHole; shots: RoundShotSnapshot[]; unitEvents: UnitEvent[]; counterBetEvents: CounterBetEvent[]; counterBetKeepers: CounterBetKeepers; lobaHoles: Record<number, LobaHole>; manualBets: ManualBet[]; ballFriendSetup: Record<number, BallFriendHole> }>>([]);
  ...
  const order = useMemo(() => playOrderForHoles(course.holes.map((hole) => hole.number), startHole).slice(0, roundHoles), [course.holes, startHole, roundHoles]);
  ...
  const applyDraft = useCallback((value: unknown, options: { preserveLocalUi?: boolean } = {}) => {
    ...
    if (draft && draftCore) {
        ...
        setShots(Array.isArray(draft.shots) ? draft.shots as RoundShotSnapshot[] : []);
  ...
  const liveScoreboard = useMemo(() => buildLiveScoreboard({ course, players, scores: liveScores, order, deterministic: { engineVersion: "phase1-canonical-v1", balances: liveBetBalances, calculatedAt: roundStartedAt || `${roundDate}T12:00:00-06:00` } }), [course, liveBetBalances, liveScores, order, players, roundDate, roundStartedAt]);
  const liveOwnerPutts = useMemo(() => order.reduce((total, playedHole) => {
    const value = putts[playedHole]?.[ownerId];
    return total + (typeof value === "number" ? value : 0);
  }, 0), [order, ownerId, putts]);
  const liveQuestionGameFacts = useMemo(() => {
    const balanceLeader = [...players].sort((left, right) => (liveBetBalances[right.id] || 0) - (liveBetBalances[left.id] || 0))[0];
    const skinsLeader = [...players].sort((left, right) => (skins.won[right.id] || 0) - (skins.won[left.id] || 0))[0];
    return {
      leaderName: balanceLeader?.name || null,
      leaderBalance: balanceLeader ? liveBetBalances[balanceLeader.id] || 0 : null,
      ownerSkins: skins.won[ownerId] || 0,
      skinsLeaderName: skinsLeader && (skins.won[skinsLeader.id] || 0) > 0 ? skinsLeader.name : null,
      skinsLeaderCount: skinsLeader ? skins.won[skinsLeader.id] || 0 : 0,
      camelsEvents: camels.totalQuantity,
      fishEvents: fish.totalQuantity,
      vipersEvents: vipers.totalQuantity,
    };
  }, [camels.totalQuantity, fish.totalQuantity, liveBetBalances, ownerId, players, skins.won, vipers.totalQuantity]);
  ...
  function editHistoricalRound(snapshot: RoundSnapshot) {
    ...
    setScoreCaptureMode(normalizeScoreCaptureMode(restored.scoreCaptureMode)); setAdvancedStats(normalizeAdvancedStats(restored.advancedStats)); setShots(restored.shots || []);
  ...
  function applyScannedScorecard(
    ...
    if (!result.ready || roundClosed || roundId.startsWith("shared:") || history.some(round => round.id === roundId && round.cloudReadOnly) || !ownsLocalWorkspace(localStorage, identity.userId)) return false;
  ...
  function selectRoundCourse(nextCourse: Course, returnToSetup = false, suppliedCards?: Course[]) {
    const cards = usableRoundCourseCards(suppliedCards?.length ? suppliedCards : teeOptionsForCourse(nextCourse, courseOptions));
    const transition = changeRoundCourseSelection(courseSelected ? course : null, cards);
    if (!transition.ok) {
      setCourseSelectionError(true);
      setFeedback("Este recorrido no tiene una tarjeta utilizable de 9 o 18 hoyos. Puedes solicitar su revisión.");
      return;
    }
    const apply = () => {
      retainCatalogCards(transition.teeOptions);
      setCourse(transition.course);
      if (!transition.selectedTeeId) setPlayerTeeAssignments([]);
      setCourseSelected(Boolean(transition.selectedTeeId));
      setCourseSetupStage("tee");
      setPendingCourseIdentity({
        name: transition.course.name,
        ...(transition.course.catalogCourseId ? { catalogCourseId: transition.course.catalogCourseId } : {}),
        candidateCourseIds: transition.teeOptions.map(option => option.id),
      });
      setCourseSelectionError(false);
      setRecentCourseIds((current) => rememberRecentCourse(current, transition.course.id));
      if (returnToSetup) setTab("setup");
    };
    if (transition.course.id === course.id || !courseSelected) apply();
  ...
  function selectRoundTee(nextTee: Course) {
    ...
    if (selectedTee.id === course.id) apply();
  ...
  function saveFrequentGroup() {
    ...
    const gameTemplate = templateWithoutPlayerAssignments(createGroupGameTemplate({ ownerId, players, startHole, roundHoles, roundHandicapBasis, bets, segments, personalBets, supplementalBets, manualBets }, Object.fromEntries(mapped.map(({ player, memberId }) => [player.id, memberId]))));
  ...
  function retainCatalogCards(cards:Course[]) {
    const usable=usableRoundCourseCards(cards);
  ...
  async function saveFrequentGroupEdit() {
    ...
    const normalizedTemplate = normalizeGroupGameTemplate(stableDraft.gameTemplate, stableDraft.players) ?? createEmptyGroupGameTemplate(stableDraft);
    const templateIssues = groupTemplateConfigurationIssues(normalizedTemplate, groupTemplatePlayers(stableDraft));
    if (templateIssues.blocking.length) {
      setFrequentGroupEditError(templateIssues.blocking.map((issue) => issue.message).join(" "));
      setFrequentGroupEditorTab("bets");
      return;
    }
    const savedDraft = { ...stableDraft, gameTemplate: templateWithoutPlayerAssignments(normalizedTemplate), updatedAt: now };
    ...
    frequentGroupSaveInFlight.current = true;
    setFrequentGroupSaving(true);
    setFrequentGroupEditError("");
    const savingUserId = identity.userId;
    try {
      const saved = await saveExplicitGroupSnapshot({ group: savedDraft, accessToken: identity.accessToken, authenticated: identity.mode === "authenticated", online: navigator.onLine });
      if (liveIdentity.current.userId !== savingUserId) return;
      setFrequentGroups((groups) => frequentGroupDraftIsNew
        ? [saved.group, ...groups.filter((group) => group.id !== saved.group.id)]
        : updateFrequentGroupTemplate(groups, saved.group.id, saved.group, now));
      ...
      frequentGroupSaveInFlight.current = false;
      setFrequentGroupSaving(false);
    }
  }
  ...
  return <main className={`app ${highContrast ? "highContrast" : ""} ${tab === "results" ? "compactResults" : ""} ${tab === "welcome" ? "homeApp" : ""}`}>
    ...
    {tab === "aiSetup" && <AiRoundSetup
      ...
      onOpenPrivacy={() => { setOpenAiPrivacySettings(true); setTab("account"); }}
    />}
    ...
    {tab === "scorecardScan" && <ScorecardScanner
      ...
      round={{
        roundId,
        revision: JSON.stringify({ draft: roundDraftPayload(), roundClosed, readOnly: roundId.startsWith("shared:") || history.some(round => round.id === roundId && round.cloudReadOnly), userId: identity.userId }),
    ...
    {tab === "setup" && <RoundSetupWizard key={`${identity.userId}:${roundId}`} storageKey={`backyard-setup-step-v1:${identity.userId}:${roundId}`} issues={roundSetupPreflight} editing={editingRound} scoreOnly={roundPresentation.playMode === "score_only"} initialStep={roundSetupInitialStep}
      ...
      <section className="card" id="round-course">
        ...
        {courseSetupStage === "course" && <>
          ...
          <details><summary>Mis campos guardados</summary><RoundCoursePicker key={`round-saved-${identity.userId}`} permissionOwnerId={identity.userId} selectedName="" selectedId="" accessToken={identity.accessToken} pendingName={pendingCourseIdentity?.name} invalid={courseSelectionError} describedBy={[!courseSelected && pendingCourseIdentity ? "round-course-ai-focus" : "", courseSelectionError ? "round-course-error" : ""].filter(Boolean).join(" ") || undefined} onSelect={(selection) => {
            ...
            if (matchingCourse) selectRoundCourse(matchingCourse, false, teeOptionsForCourse(matchingCourse, courseOptions));
          }} /></details>
        ...
        {courseSetupStage === "tee" && <RoundTeePicker
          ...
          tees={teeOptions}
          selectedTeeId={selectedRoundTeeId}
          onSelect={selectRoundTee}
          onBack={() => setCourseSetupStage("course")}
      ...
      <section className="card">
        ...
        <SetupBetCard id="units" icon="📏" title="Unidades positivas y negativas" description="Puntos positivos y negativos · todos contra todos" help="units" enabled={bets.units.enabled} locked={!bettingConsentGranted} requestActivation={requestBettingConsent} onEnabledChange={(enabled) => setBets((current) => ({ ...current, units: { ...current.units, enabled } }))}><div className="grid2"><MoneyInput label="Valor por unidad positiva" value={bets.units.value} onChange={(v) => setBets({ ...bets, units: { ...bets.units, value: v } })} /><MoneyInput label="Valor por unidad negativa" value={bets.units.copaValue ?? bets.units.value} onChange={(v) => setBets({ ...bets, units: { ...bets.units, copaValue: v } })} /></div><label className="miniLabel">Participan</label><ParticipantChips players={players} selected={bets.units.participantIds} onChange={(ids) => setBets({ ...bets, units: { ...bets.units, participantIds: ids } })} /></SetupBetCard>
    ...
    {tab === "round" && <>
      ...
      <RoundCaptureV2
        ...
        scores={Object.fromEntries(players.map((player) => {
          const editedRow = scoreEdits[holeNumber];
          const value = editedRow && Object.hasOwn(editedRow, player.id)
            ? editedRow[player.id]
            : scores[holeNumber]?.[player.id];
        ...
        onShotsChange={(nextShots) => { const added = nextShots.find((shot) => !shots.some((current) => current.id === shot.id)); checkpoint(); setShots(nextShots); if (added) void recordProductEvent({ eventId: `shot-${added.id}`, eventName: "shot_recorded", accessToken: identity.accessToken, occurredAt: added.startedAt, metadata: { source: added.source, quantity: 1 } }); }}
      ...
      {holeBetEditor && <div className="modalBackdrop holeBetModalBackdrop" role="presentation"><section className="confirmDialog holeBetEditorDialog" role="dialog" aria-modal="true" aria-labelledby="hole-bet-editor-title">
        <ModalCloseButton onClose={() => setHoleBetEditor(null)} />
    ...
    {tab === "standings" && <>
      ...
      <LiveRoundQuestion playerId={ownerId} scoreboard={liveScoreboard} putts={liveOwnerPutts} gameFacts={liveQuestionGameFacts} consentOwnerId={identity.userId || undefined} accessToken={identity.accessToken} requiresRemoteConsent={identity.mode === "authenticated"} />
<section className="card"><h2>Desglose por apuesta</h2><div className="tableWrap"><table><thead><tr><th>Jugador</th><th>🐇</th><th>⛳</th><th>📏</th><th>🤝 Foursome</th><th>⚪🤝 Bola Amiga</th><th>🏆 {groupNassauLabels.name}{bets.miniPolla.enabled ? " / Mini Polla" : ""}</th>{liveSupplementalGeneralResults.map((result) => <th key={result.betId}>{supplementalBetDisplayLabel(result.type, result.label)}</th>)}<th>↔ Personales</th><th>✍️ Manuales</th>{bets.monkey?.enabled && <th>🐒 Monkey</th>}{bets.vipers.enabled && <th>🐍</th>}{bets.camels.enabled && <th>🐫</th>}{bets.fish.enabled && <th>🐟</th>}{bets.loba.enabled && <th>🐺</th>}</tr></thead><tbody>{players.map((player) => <tr key={player.id}><td><b>{player.name}</b></td><td>{signedMoney(rabbitBalances[player.id] || 0)}</td><td>{signedMoney(skinBalances[player.id] || 0)}</td><td>{signedMoney(units.balances[player.id] || 0)}</td><td>{signedMoney(foursomes.provisionalBalances[player.id] || 0)}</td><td>{signedMoney(ballFriend.balances[player.id] || 0)}</td><td>{signedMoney((polla.balances[player.id] || 0) + (miniPolla.balances[player.id] || 0))}</td>{liveSupplementalGeneralResults.map((result) => <td key={result.betId}>{signedMoney(result.balances[player.id] || 0)}</td>)}<td>{signedMoney(ownerId === player.id ? livePersonalOpponentResults.reduce((sum, entry) => sum + entry.amount, 0) : livePersonalOpponentResults.filter((entry) => entry.opponentId === player.id).reduce((sum, entry) => sum - entry.amount, 0))}</td><td>{signedMoney(manual.balances[player.id] || 0)}</td>{bets.monkey?.enabled && <td>{signedMoney(monkey.balances[player.id] || 0)}</td>}{bets.vipers.enabled && <td>{signedMoney(vipers.balances[player.id] || 0)}</td>}{bets.camels.enabled && <td>{signedMoney(camels.balances[player.id] || 0)}</td>}{bets.fish.enabled && <td>{signedMoney(fish.balances[player.id] || 0)}</td>}{bets.loba.enabled && <td>{signedMoney(loba.balances[player.id] || 0)}</td>}</tr>)}</tbody></table></div></section>
    ...
    {tab === "results" && <>
      ...
      <ResultAccordion id="bet-values" title="Valores de apuesta" className="betValues" {...resultAccordionProps("bet-values")}><div className="valueGrid">
        ...
        {bets.units.enabled && <span><b>📏 Unidades positivas y negativas</b>{money(bets.units.value)} por positiva · {money(bets.units.copaValue ?? bets.units.value)} por negativa</span>}
...
<div className="roundActions">{(roundReviewPending || roundClosed) && <button className="secondary big" onClick={async () => { const snapshot = currentSnapshot(); if (snapshot) await shareRound(snapshot); }}>Compartir resultado</button>}{!roundReviewPending && <button className="secondary big" onClick={requestNewRound}>Nueva ronda</button>}{roundClosed && <button className="secondary big" onClick={() => { const snapshot = currentSnapshot(); if (snapshot) { setHistoryDetailId(snapshot.id); setTab("historyDetail"); } }}>Compartir con jugadores</button>}{roundClosed ? <button className="primary big" onClick={() => setTab("history")}>Abrir Histórico</button> : roundReviewPending ? <button className="primary big" disabled={saveStatus === "saving"} onPointerDown={commitFocusedNumericCapture} onClick={requestRoundHistorySave}>Guardar en Histórico</button> : <button className="primary big" onClick={openActiveRound}>Volver a la ronda</button>}</div>
    ...
    {tab === "history" && <>
      ...
      {Object.keys(historicalGolfInsights.categoryTotals).length > 0 && <ResultAccordion id="history-bet-balance" title="Balance por apuesta"><div className="expenseBars">{Object.entries(historicalGolfInsights.categoryTotals).map(([name, value]) => <div key={name}><span>{historicalBetDisplayLabel(name)}</span><b className={value > 0 ? "good" : value < 0 ? "bad" : ""}>{signedMoney(value)}</b></div>)}</div></ResultAccordion>}
...
export default function Page() {
  return <><CaptureGroupInvitationLink /><AccountProvider><PendingGroupInvitation /><GolfBetsApp /></AccountProvider></>;
}
