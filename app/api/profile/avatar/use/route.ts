...
import { commitGeneratedAvatar } from "../../../../../lib/avatar-generation-service";
...
export async function POST(request: NextRequest) {
  ...
  if (!read.ok) return json({ code: read.reason === "too_large" ? "REQUEST_TOO_LARGE" : "INVALID_REQUEST", error: read.reason === "too_large" ? "El avatar generado es demasiado grande." : "Solicitud inválida." }, read.reason === "too_large" ? 413 : 400);
  const result = await commitGeneratedAvatar(read.value, avatarGenerationDeps(request));
