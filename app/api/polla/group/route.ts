...
export async function GET(request: NextRequest) {
  ...
  const [{ data: group }, { data: members }, { data: scores }] = await Promise.all([
    admin.from("tournament_groups").select("id,name,start_hole,status,last_score_at,tournaments(name,holes,course_snapshot)").eq("id", access.group_id).single(),
