...
const rawRequest=credentialBoundFetch(c.previewOrigin),databaseFetch=credentialBoundFetch(c.supabaseOrigin);await verifyPreviewBundleBinding(c,rawRequest,databaseFetch);const request=deploymentMutationBoundFetch(c,rawRequest);
...
const client=createClient(c.supabaseOrigin,c.publicKey,{auth:{persistSession:false,autoRefreshToken:false},global:{fetch:databaseFetch}});
