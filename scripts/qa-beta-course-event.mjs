...
const people=JSON.parse(readFileSync('.qa-artifacts/beta-fixtures.private.json','utf8'));
for(const a of people){assert.equal(a.ref,config.projectRef);assert.equal(a.preview,config.previewOrigin);a.client=createClient(config.supabaseOrigin,config.publicKey,{auth:{persistSession:false,autoRefreshToken:false},global:{fetch:credentialBoundFetch(config.supabaseOrigin)}});const r=await a.client.auth.signInWithPassword({email:a.email,password:a.password});assert.equal(r.error,null);assert.equal(r.data.user.id,a.id);assert.equal(r.data.user.app_metadata.qa_run_id,a.runId);a.token=r.data.session.access_token;}
...
try {
 ...
 const base=(await app('/api/cloud/rounds',a)).rounds.find(r=>r.lifecycleState==='completed');assert.ok(base);
...
const report={preview:config.previewOrigin,ref:config.projectRef,passed,failure,qualification:'Real Preview API + QA DB; synthetic rounds referencing canonical catalog IDs, not a claim of a real played round.'};
