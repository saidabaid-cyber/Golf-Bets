...
try{
  ...
  const response=transport.normalizeBallFitApiSuccess(await app('/api/ball-fitting','POST',{input:requestInput}));assert.ok(response);assert.ok(response.result.recommendations.length>0);assert.equal(response.scope.complete,true);
  fitCatalogEvidence={provider:response.provider,scope:response.scope,status:response.result.status,recommendationIds:response.result.recommendations.map(item=>item.catalogBallId)};
  const summary=fit.toEquipmentBallFitSummary(response.result,randomUUID(),new Date().toISOString(),input);assert.ok(summary);profile=eq.setLastBallFit(profile,summary);await save(profile);passed.push('BALL_FIT_REAL_INTERNAL_CATALOG_RESPONSE','BALL_FIT_SAVE_INPUT_RESULT_CLOUD');
  stage='FRESH_SESSION';await account.client.auth.signOut();await login();assert.deepEqual((await app('/api/equipment')).data.profile,profile);passed.push('BALL_SHAFT_DISTANCE_FIT_FRESH_SESSION_PERSISTENCE');
  stage='FREEZE_ROUND_SHOT';
  const choice=capture.captureClubChoices(profile.clubs[0])[0],localRoundId=`qa-fit-round-${runId}`,playerId=`qa-player-${account.id}`;at=new Date().toISOString();const holes=Array.from({length:18},(_,i)=>({number:i+1,par:4,strokeIndex:i+1}));
  const snapshot={id:localRoundId,ownerId:playerId,ownerName:account.name,accountUserId:account.id,lifecycleState:'completed',startedAt:at,completedAt:at,date:at.slice(0,10),roundHoles:18,courseName:'Synthetic QA fitting course — not real',teeName:'QA unverified',courseSnapshot:{id:`qa-course-${runId}`,name:'Synthetic QA fitting course — not real',teeName:'QA unverified',holes},order:holes.map(h=>h.number),players:[{id:playerId,accountUserId:account.id,name:account.name,handicap:0}],scores:Object.fromEntries(holes.map(h=>[h.number,{[playerId]:4}])),shots:[{id:randomUUID(),roundId:localRoundId,playerId,hole:1,sequence:1,clubId:choice.id,clubLabel:choice.label,clubSnapshot:choice,source:'MANUAL',startedAt:at,distanceYards:200}],ownerBagSnapshot:[choice.label],expenses:{caddie:0,food:0,drinks:0,greenFee:0,cartRental:0,other:0},expenseTotal:0,betResult:0,netResult:0,categoryResults:{}};
  roundId=(await app('/api/cloud/rounds','POST',{round:snapshot},201)).roundId;
  const original=checked(await account.client.from('rounds_cloud').select('snapshot').eq('id',roundId).single(),'snapshot read').snapshot;assert.deepEqual(original.shots,snapshot.shots);passed.push('ROUND_CLUB_SHAFT_SNAPSHOT_CLOUD');
  stage='EDIT';at=new Date().toISOString();profile=eq.upsertPlayerClub(profile,{...profile.clubs[0],customShaftModel:'QA Shaft V2',flex:'R',shaftFlexLabel:'R',shaftWeightGrams:55,updatedAt:at},at);profile=eq.upsertPlayerBall(profile,{...profile.balls[0],color:'YELLOW',updatedAt:at},at);profile=eq.upsertPlayerClubDistance(profile,{...profile.distances[0],carryDistance:210,totalDistance:230,updatedAt:at},at);await save(profile);
  assert.equal(record.profile.clubs[0].customShaftModel,'QA Shaft V2');assert.equal(record.profile.balls[0].color,'YELLOW');assert.equal(record.profile.distances[0].carryDistance,210);passed.push('BALL_UPDATE_CLOUD','SHAFT_UPDATE_CLOUD','DISTANCE_UPDATE_CLOUD');
  input={...input,feelPreference:'FIRM',colorPreference:'YELLOW'};const revised=transport.normalizeBallFitApiSuccess(await app('/api/ball-fitting','POST',{input:transport.createBallFitTransportInput(input)}));assert.ok(revised);const revisedSummary=fit.toEquipmentBallFitSummary(revised.result,randomUUID(),new Date().toISOString(),input);assert.ok(revisedSummary);profile=eq.setLastBallFit(profile,revisedSummary);await save(profile);assert.equal(record.profile.lastBallFit.input.feelPreference,'FIRM');passed.push('BALL_FIT_UPDATE_INPUT_RESULT_CLOUD');
  assert.deepEqual(checked(await account.client.from('rounds_cloud').select('snapshot').eq('id',roundId).single(),'frozen after edit').snapshot,original);passed.push('ROUND_SHAFT_SNAPSHOT_UNCHANGED_AFTER_BAG_EDIT');
