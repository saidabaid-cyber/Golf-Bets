...
try {
  if (args.includes('--help')) console.log('backup [--only=source|schema|database|storage]. Environment only; explicit local/qa/owner allowlist, read-only export, encryption required for DB/Storage. Never deploys, restores or mutates the source. See docs/disaster-recovery/README.md. Exit 0 complete, 2 incomplete, 1 failed.');
  else {
