...
test('environment example documents runtime variables without committing secret values',async()=>{
  const repo=fileURLToPath(new URL('../../',import.meta.url));
  const lines=(await readFile(join(repo,'.env.example'),'utf8')).split(/\r?\n/)
    .map(line=>line.trim()).filter(line=>line&&!line.startsWith('#'));
  assert.ok(lines.every(line=>/^[A-Z][A-Z0-9_]*=.*$/.test(line)));
  const assignments=new Map(lines.map(line=>{const separator=line.indexOf('=');return[line.slice(0,separator),line.slice(separator+1)];}));
  assert.equal(lines.length,assignments.size);
  const names=[...assignments.keys()];
  for(const name of ['BACKUP_PGPASSWORD','BACKUP_STORAGE_KEY','BACKUP_ENCRYPTION_KEY','GDRIVE_SERVICE_ACCOUNT_JSON'])
    assert.equal(assignments.get(name),'',name);
  assert.equal(assignments.get('BACKUP_RETENTION_APPLY'),'false');
  const document=await readFile(join(repo,'docs/disaster-recovery/ENVIRONMENT_VARIABLES.md'),'utf8');
  for(const name of ['BACKUP_SOURCE','BACKUP_EXPECTED_REF','BACKUP_PGHOST','BACKUP_PGPORT','BACKUP_PGUSER',
    'BACKUP_PGDATABASE','BACKUP_PGSSLROOTCERT','BACKUP_STORAGE_URL','BACKUP_PGPASSWORD','BACKUP_STORAGE_KEY',
    'BACKUP_ENCRYPTION_KEY','BACKUP_ROOT','BACKUP_DECRYPT_ACK','GDRIVE_SERVICE_ACCOUNT_JSON',
    'GDRIVE_BACKUP_ROOT_FOLDER_ID','BACKUP_RETENTION_APPLY']) {
    assert.ok(names.includes(name),name);assert.ok(document.includes('`'+name+'`'),name);
  }
});
test('generic local tools never inherit backup or Google Drive credentials',()=>{
  const safe=commandEnvironment({PATH:'synthetic-path',BACKUP_PGPASSWORD:'private',backup_storage_key:'private',GDRIVE_SERVICE_ACCOUNT_JSON:'private',OTHER:'public'});
  assert.deepEqual(safe,{PATH:'synthetic-path',OTHER:'public'});
});
